...
class ParseService:
    def __init__(self, db_service: DBService, logging_service: LoggingService, rabbitmq_service: RabbitMQService):
    ...
    def parse_body(self, email_id, email_body):
        ...
        if((len(all_titles) != 0) and (len(all_snippets) != 0) and (len(all_titles) != len(all_snippets))):
            self.raise_google_scholar_format(email_id, body_text, "Problem with Google Scholar classes: gse_alrt_title, gse_alrt_sni: ")
    ...
    def add_to_queue(self, search_result_id):
        exchange=''
        routing_key='doi'
        body=str(search_result_id)
        self.rabbitmq_service.post(exchange, routing_key, body, self.logging_service)
